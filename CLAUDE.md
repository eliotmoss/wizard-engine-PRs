...
| `X86_64PersistentExplorer.v3` | Schedule enumeration over a trace: `PersistentExplorer.crashImages()` (full branching search with state dedup and a reported budget) and `reducedCrashImages()` (fence-forced floor + per-line prefix product); `PersistentExploration` result/artifact |
...
| `test/unittest/x86-64-linux/PersistentExplorerTest.v3` | Crash-image enumeration tests (image counts per cut, reduction agrees with full search, budget truncation) |
