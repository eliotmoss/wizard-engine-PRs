...
| `scripts/stage2c.sh` | Stage 2c operator script (`doctor`/`probe`/`probe-now`/`sieve`/`verify`); refuses to arm while the firmware's memory-overwrite request is set; host setup, runbook and cross-machine hand-off in `docs/stage2c-handoff.md` |
