...
| Stage 2c — reserved-DRAM warm reboot | ~2–3 days, risky | Real cache loss on a DAX-faithful stand-in. Bare-metal host is available (confirmed 2026-09-18). **Hard timebox: if it is not working by 2026-10-02, drop it** and present the negative control as the sole flush-placement evidence. **First reading 2026-09-23: probe `NOT-SENSITIVE`**, after the firmware's memory-overwrite wipe was found and cleared; a zero-window probe decides whether the reset writes the cache back. See `docs/stage2c-handoff.md`. |
