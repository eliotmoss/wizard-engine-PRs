...
| Log-full handling | `append()` silently drops the entry when the log is full (correctness hole) | `reserveRecord` wraps around and triggers a lazy checkpoint; `commit()` returns `0` if a record still cannot fit | Per-transaction capacity is fixed at `slotBytes` (~half the chunk); an oversize commit fails cleanly with `0` before touching the slot. An invalid `append()` poisons the transaction so `commit()` fails |
| `commit()` signature | `commit()` → `void` (persist implied) | `commit()` → `u64` (returns the assigned `txnSeq`, `0` on failure) | `commit()` → `u64`; empty commits write an `entryCount=0` record so success is always nonzero. `recover()` → `DualWalRecovery` (`CLEAN`/`REPLAYED`/`CORRUPT`/`PERSIST_FAILED`) |
