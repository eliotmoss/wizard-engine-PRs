...
**Audited 2026-07-31 and extended 2026-08-06:** the four implementation-specific x86-64 Linux test files contain **118 registered tests**. All 118 pass with no expected failures when the x86-64 Linux unit binary is run in an amd64 Docker container on the current Darwin arm64 host.
...
- `TxnPWRegionTest.v3` — **46 tests** across allocator (`pwregion:`), Immix metadata (`pwregion_immix:`), overflow propagation (`pwregion_overflow:`), injected recovery-required propagation (`pwregion_recovery:`), backend ownership/state (`txn_backend:`), and file-backed remount/recovery (`pwregion_bd:`): format, alloc/free/coalescing/exhaustion, allocation/free persistence-failure propagation, rejection of zero/oversized allocations and null/double/foreign frees, descriptor-driven line geometry and chunk-to-line-mark linkage, `DualTxnWal` recovery/checksum rejection, abrupt termination before or after allocator after-image application (including split and exact-fit allocation recovery, and `SIGKILL` after application), mmap/PMEM state and lifecycle, and file-backed persistence
