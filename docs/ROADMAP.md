...
- Tests: `DualTxnWalTest.v3` grew to 38 cases — crash after apply/before next commit, back-to-back commits then crash, one-boundary-per-commit + induction property (counting region: 1 `persistChanges`, 0 `persistRange` per commit), explicit-flush durability, commit/apply/recovery/flush/final-data-close/slot-scrub failure requiring reopen, fresh-header fail-before-copy and copy-then-fail outcomes, close-scrubs-for-clean-remount, close-keeps-unrecovered-records, a shadow durable-memory model with fail-before/copy-then-fail/partial-copy outcomes, mixed-width and entry-boundary validation, and checksummed malformed record-header/trailer rejection; the copy-then-fail regressions make indeterminate persistence executable. `RegionTransactionTest.v3`: boundary counts re-pinned (record+data prepares, 1 `persistChanges`/commit), piggybacked-durability and `flush()` tests added
...
**Audited 2026-07-31 and extended 2026-08-11:** the five implementation-specific x86-64 Linux test files contain **151 registered tests**. The previous 140-test audit passed in an amd64 Docker container on a Darwin arm64 host; all 151 pass with no expected failures in the latest native x86-64 Linux run.

- `DualTxnWalTest.v3` — **38 tests** for the active two-slot WAL: format/geometry, fresh-header persistence failures, slot alternation, empty commits, crash recovery against separate live/durable byte images (including 2/4/8-byte and signed-value after-images), entry-range and WAL-overlap rejection, corrupt/torn records, checksummed malformed record-header/trailer rejection, replay ordering, overwrite guard, oversize/poisoned commits, commit/apply/recovery/flush/final-data-close/slot-scrub recovery-required enforcement, fail-before/copy-then-fail/partial-copy shadow outcomes, the normative unacknowledged-record replay, the phase-B one-boundary induction property, explicit flush, and clean/unrecovered close paths
...
- `TxnPWRegionTest.v3` — **65 tests** across allocator (`pwregion:`), Immix metadata (`pwregion_immix:`), overflow propagation (`pwregion_overflow:`), injected recovery-required propagation (`pwregion_recovery:`), backend ownership/state (`txn_backend:`), and file-backed remount/recovery (`pwregion_bd:`): format, fresh truncation/zero-fill and missing-file attach fallback, stored-geometry validation and legacy WAL-offset fallback, alloc/free/coalescing/exhaustion, direct memory-order/free-list invariant walking over hand-written and reproducibly generated mixed allocate/free/remount histories, allocation/free persistence-failure propagation, rejection of zero/oversized allocations and null/double/foreign frees, descriptor-driven line geometry and chunk-to-line-mark linkage, `DualTxnWal` recovery/checksum rejection, real-file `fdatasync` commit ordering and injected-error recovery, page-aligned `msync` format ordering and clean-close failure recovery, abrupt termination before or after allocator after-image application (including deterministic `SIGKILL` at the split, exact-fit, coalescing-free, N+1 piggyback, and explicit-flush boundaries), mmap/PMEM state and lifecycle, and file-backed persistence
