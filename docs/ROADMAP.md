...
**Audited 2026-07-31 and extended 2026-08-06:** the four implementation-specific x86-64 Linux test files contain **127 registered tests**. All 127 pass with no expected failures when the x86-64 Linux unit binary is run in an amd64 Docker container on the current Darwin arm64 host.
...
- [ ] Add abrupt-process tests for the file backend. Run the writer in a child process, terminate with `_exit`/`SIGKILL` at WAL protocol boundaries without `deallocate()`/`close()`, then reopen and verify in a separate process. Trace or intercept `fdatasync`/`msync` to check ordering and inject errors. This establishes independence from graceful shutdown and exercises the real syscall translation, but must be described as process-crash consistency rather than host power-loss proof. Seven slices complete: `pwregion_bd:abrupt_writer_recovers_committed_wal` exits after a committed record but before after-image application; the split and exact-fit commit-boundary tests stop inside the real file backend immediately after the allocation's commit-point `fdatasync`, then the parent sends `SIGKILL` before `persistChanges()` returns or after-images apply; the split, exact-fit, and coalescing-free tests exit after normal allocator commits have applied their deferred after-images; and `pwregion_bd:sigkill_split_alloc_recovers_transaction` deterministically stops at that applied-after-image window before the parent sends `SIGKILL`. All remount from a fresh parent-process mapping without graceful child cleanup. Remaining work covers other protocol windows, syscall ordering, and injected syscall failures.
- [ ] Recover complete multi-entry allocator transactions, not only a manually injected one-byte `BlockEntry.used` update. Cover allocation split/exact-fit and free left/right coalescing at the commit→crash→recover and apply→crash→recover windows, then validate all memory-order and free-list links. Two commit→crash→recover slices are complete: `pwregion_bd:sigkill_split_alloc_commit_boundary_recovers` checks the full split transaction, while `pwregion_bd:sigkill_exact_fit_alloc_commit_boundary_recovers` checks the distinct no-remainder and empty-free-list state after stopping at the real file-backend commit boundary. Three apply→crash→recover slices are complete: `pwregion_bd:abrupt_split_alloc_recovers_transaction` checks split allocation's allocated/remainder links; `pwregion_bd:abrupt_exact_fit_alloc_recovers_transaction` checks its distinct empty-list state; and `pwregion_bd:abrupt_coalescing_free_recovers_transaction` checks a free that coalesces both left and right neighbors back into the original five-block extent. Each checks memory-order links, free-list links, used/list state, and transaction health directly after remount.
