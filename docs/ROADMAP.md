...
**Audited 2026-07-31 and extended 2026-08-06:** the four implementation-specific x86-64 Linux test files contain **130 registered tests**. All 130 pass with no expected failures when the x86-64 Linux unit binary is run in an amd64 Docker container on the current Darwin arm64 host.

- `DualTxnWalTest.v3` — **36 tests** for the active two-slot WAL: format/geometry, fresh-header persistence failures, slot alternation, empty commits, crash recovery against separate live/durable byte images (including 2/4/8-byte and signed-value after-images), entry-range and WAL-overlap rejection, corrupt/torn records, replay ordering, overwrite guard, oversize/poisoned commits, commit/apply/recovery/flush/final-data-close/slot-scrub recovery-required enforcement, fail-before/copy-then-fail/partial-copy shadow outcomes, the normative unacknowledged-record replay, the phase-B one-boundary induction property, explicit flush, and clean/unrecovered close paths
...
- [ ] Add active commit/recovery coverage for widths 2/4/8 and signed `i16`/`i64`, out-of-region and WAL-overlapping entries, and malformed record header/trailer fields—not only checksum corruption. The mixed-width recovery slice is complete: `dual_wal:mixed_width_signed_values_recover` commits one 2/4/8-byte record, crashes before application, replays signed `i16`/`i64` bit patterns, and proves replay persistence with a second crash. Entry ranges are covered by `dual_wal:invalid_entry_ranges_rejected`, including end-of-region crossings, overflow-safe rejection, every WAL boundary crossing, and successful recovery at both exact adjacent offsets. Malformed-record cases remain open.
