...
**Audited 2026-07-31 and extended 2026-08-06:** the five implementation-specific x86-64 Linux test files contain **137 registered tests**. All 137 pass with no expected failures when the x86-64 Linux unit binary is run in an amd64 Docker container on the current Darwin arm64 host.
...
- `TxnPWRegionTest.v3` — **56 tests** across allocator (`pwregion:`), Immix metadata (`pwregion_immix:`), overflow propagation (`pwregion_overflow:`), injected recovery-required propagation (`pwregion_recovery:`), backend ownership/state (`txn_backend:`), and file-backed remount/recovery (`pwregion_bd:`): format, fresh truncation/zero-fill and missing-file attach fallback, stored-geometry validation and legacy WAL-offset fallback, alloc/free/coalescing/exhaustion, direct memory-order/free-list invariant walking over a mixed allocate/free/remount history, allocation/free persistence-failure propagation, rejection of zero/oversized allocations and null/double/foreign frees, descriptor-driven line geometry and chunk-to-line-mark linkage, `DualTxnWal` recovery/checksum rejection, abrupt termination before or after allocator after-image application (including deterministic `SIGKILL` at the split, exact-fit, and coalescing-free transactions' commit-before-apply boundary, split/exact-fit allocation and coalescing-free recovery after application, and `SIGKILL` after application), mmap/PMEM state and lifecycle, and file-backed persistence
...
- [ ] Add structural/property tests over mixed allocate/free/remount sequences so allocator list invariants are checked directly instead of inferred only from whether a later allocation succeeds. Initial deterministic slice complete: `pwregion_bd:mixed_sequence_preserves_invariants` walks the full memory-order chain and both free lists after each split/free/coalesce mutation and after three remounts, checking range/order/reciprocal-link, extent-classification, uniqueness, membership, and transaction-health invariants. Seeded/generated histories remain open.
