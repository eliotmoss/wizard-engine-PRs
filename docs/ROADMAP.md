...
- The current implementation still permits a same-slot retry after a failed boundary: the sequence is not burned and parity maps the retry onto the same slot. This is retained as a characterization of the pre-contract implementation, not the target behavior; the recovery-required latch in Next Steps #3 must reject that retry
- Persistence-failure contract chosen 2026-07-30: a boundary-related `commit() == 0` is **unacknowledged**, not a guaranteed abort. The mount becomes recovery-required; recovery may replay any complete durable record because entries are idempotent after-images. Enforcement of the no-further-operations rule remains in Next Steps #3
- Tests: `DualTxnWalTest.v3` grew to 26 cases — crash after apply/before next commit, back-to-back commits then crash, one-boundary-per-commit + induction property (counting region: 1 `persistChanges`, 0 `persistRange` per commit), explicit-flush durability, close-scrubs-for-clean-remount, close-keeps-unrecovered-records, and a shadow durable-memory model with fail-before/copy-then-fail/partial-copy outcomes; the copy-then-fail regression is now the normative recovery-required outcome. `RegionTransactionTest.v3`: boundary counts re-pinned (record+data prepares, 1 `persistChanges`/commit), piggybacked-durability and `flush()` tests added
...
- Failed commit-point persist burns nothing: the seq is not advanced and parity maps the retry onto the same slot, so a duplicate-seq record at another location is structurally impossible. This removes the ring WAL's head rollback requirement; whether the failed slot still needs durable invalidation before an immediate crash is tracked separately in Next Steps #3
...
- Tests: `DualTxnWalTest.v3` (fresh init, slot alternation, empty-commit seq, commit→crash→recover, clean remount, torn-record rejection, two-slot ascending replay, overwrite + latest-two replay, oversize-commit failure, poisoned-append commit failure, unrecovered-slot overwrite guard, failed-persist same-slot/same-seq retry, corrupt-header recovery result); `RegionTransactionTest.v3`/`TxnPWRegionTest.v3` re-pointed (slot geometry, boundary-2 persist counts, recovery paths)
...
**Audited 2026-07-30:** the four implementation-specific x86-64 Linux test files contain **94 registered tests**. All 94 pass; none are listed as expected failures.

- `DualTxnWalTest.v3` — **26 tests** for the active two-slot WAL: format/geometry, slot alternation, empty commits, crash recovery against separate live/durable byte images, corrupt/torn records, replay ordering, overwrite guard, oversize/poisoned commits, the legacy same-mount retry gap, fail-before/copy-then-fail/partial-copy shadow outcomes, the normative unacknowledged-record replay, the phase-B one-boundary induction property, explicit flush, and clean/unrecovered close paths
...
- **Validation direction (2026-07-30):** the test-only shadow durable-memory backend is implemented for `DualTxnWal`. It separates live bytes from a durable shadow, makes persistence operations copy between them, and restores live bytes from the shadow on simulated crash. The core crash matrix now supplies byte-level evidence for the WAL protocol under the `BackendRegion` contract and exposes the copy-then-fail result ambiguity; completing the fault matrix and allocator-level shadow factory comes before hardware-specific testing. Syscall integration, abrupt process/VM tests, and physical-media testing supply the progressively stronger outer layers of the correctness argument.
...
- [ ] Use the shadow outcomes to complete the remaining `DualTxnWal` fault-injection matrix: `DualWalRecovery.PERSIST_FAILED` + retry, failed `applyUpdate()` range preparation, failed explicit `flush()`, failed final-data persist in `close()`, failed slot-scrub persist, and fresh-header persistence failure.
...
- [ ] Enforce recovery-required state in `DualTxnWal`, `RegionTransaction`, and `PWRegion`: latch persistence-related commit/recovery/flush/close failure, reject further normal operations on that mount, and require reopen plus successful recovery before reuse. Until the API distinguishes pre-boundary validation/capacity rejection from persistence failure, higher-level `false` must conservatively require recovery.
...
| 7 | `X86_64DualTxnWal.v3:223-224` | A failed phase-B boundary can leave a complete durable record. The chosen contract permits recovery to replay this unacknowledged idempotent after-image transaction; the implementation still needs to latch recovery-required state and reject further operations on the failed mount. |
