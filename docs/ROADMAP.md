...
- Commit-originated persistence failures now latch `DualTxnWal` in a recovery-required state. The sequence is not burned, but the same instance rejects append, commit, apply, persist, recovery and close persistence work; only a newly constructed instance may inspect and recover the durable image
- Persistence-failure contract chosen 2026-07-30: a boundary-related `commit() == 0` is **unacknowledged**, not a guaranteed abort. Recovery may replay any complete durable record because entries are idempotent after-images. The core commit path enforces this rule; propagation through `RegionTransaction`/`PWRegion` and failures originating in recovery/flush/close remain in Next Steps #3
...
- [ ] Use the shadow outcomes to complete the remaining `DualTxnWal` fault-injection matrix: `DualWalRecovery.PERSIST_FAILED` + reopen/recovery on a new instance, failed `applyUpdate()` range preparation, failed explicit `flush()`, failed final-data persist in `close()`, failed slot-scrub persist, and fresh-header persistence failure.
...
- [ ] Complete recovery-required enforcement: latch failures originating directly in recovery, explicit flush, final-data close and slot scrubbing, then propagate/query the state through `RegionTransaction` and `PWRegion`. Until higher layers expose the distinction supplied by `DualTxnWal.requiresRecovery()`, their public `false` must conservatively require recovery.
...
| 7 | `X86_64DualTxnWal.v3:223-224` | Commit-originated failure now latches `DualTxnWal` and requires a new instance for recovery. Remaining work is to latch recovery/flush/close-originated failures and propagate the state through `RegionTransaction`/`PWRegion`. |
