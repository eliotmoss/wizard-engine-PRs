...
**Audited 2026-07-31 and extended 2026-08-06:** the four implementation-specific x86-64 Linux test files contain **126 registered tests**. All 126 pass with no expected failures when the x86-64 Linux unit binary is run in an amd64 Docker container on the current Darwin arm64 host.
...
- `TxnPWRegionTest.v3` — **54 tests** across allocator (`pwregion:`), Immix metadata (`pwregion_immix:`), overflow propagation (`pwregion_overflow:`), injected recovery-required propagation (`pwregion_recovery:`), backend ownership/state (`txn_backend:`), and file-backed remount/recovery (`pwregion_bd:`): format, fresh truncation/zero-fill and missing-file attach fallback, stored-geometry validation and legacy WAL-offset fallback, alloc/free/coalescing/exhaustion, direct memory-order/free-list invariant walking over a mixed allocate/free/remount history, allocation/free persistence-failure propagation, rejection of zero/oversized allocations and null/double/foreign frees, descriptor-driven line geometry and chunk-to-line-mark linkage, `DualTxnWal` recovery/checksum rejection, abrupt termination before or after allocator after-image application (including a deterministic `SIGKILL` at the split transaction's commit-before-apply boundary, split/exact-fit allocation and coalescing-free recovery after application, and `SIGKILL` after application), mmap/PMEM state and lifecycle, and file-backed persistence
