...
**Audited 2026-07-31 and extended 2026-08-06:** the four implementation-specific x86-64 Linux test files contain **116 registered tests**. All 116 pass with no expected failures when the x86-64 Linux unit binary is run in an amd64 Docker container on the current Darwin arm64 host.
...
- [ ] Cover `freeChunk()` commit-failure propagation and recovery-required behavior after a failed allocation leaves the shared transaction dirty.
